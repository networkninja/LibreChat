...
export function extractAllArtifactsFromMessage(messageText: string, messageId?: string) {
  ...
  while ((match = blockRegex.exec(messageText)) !== null) {
    ...
    console.log(`📦 [extractArtifacts] Found ${type} block ${matchCount}:`, {
      type,
    ...
    if (isUpdate) {
      // We need to UPDATE it, not create a new one
      ...
      if (!originalArtifact) {
        console.log(
          '⚠️ [extractArtifacts] artifactupdate found but no original artifact exists in current extraction',
          {
            identifier,
            messageId,
            NOTE: 'This happens when artifactupdate is in a NEW message - original artifact is in a PREVIOUS message',
            SOLUTION: 'Create update artifact with unique ID using messageId and update index',
          },
        );

        // The base artifact exists in a PREVIOUS message, and the merge logic will find it
        // Create a unique ID that includes an update marker to prevent overwriting
        ...
        const newArtifact = {
          id: updateId,
          identifier, // Keep original identifier for merge logic
        ...
        console.log('✅ [extractArtifacts] Created update artifact with unique ID:', {
          updateId,
        ...
        artifactMap.set(updateId, newArtifact);
      ...
      if (existingUpdate) {
        ...
        // Merging happens in Artifact.tsx component, not during extraction
        // Merging here causes DOUBLE MERGING (once here, once in Artifact.tsx)
      ...
      console.log('✅ [extractArtifacts] Created original artifact:', {
        id: artifact.id,
 ...
 * NOTE: Currently unused - merging now happens in Artifact.tsx component
 * Kept for reference and potential future use
 */
function _applyIntelligentMerge(
