...
export default function useArtifacts() {
  ...
  const { isSubmitting, latestMessageId, latestMessageText } = artifactsContext;
  ...
  useEffect(() => {
    const loadCache = async () => {
      console.log('🔄 [useArtifacts] STEP 1: Initializing cache from localStorage');
      artifactCache.init();

      // Load conversation cache from database on page refresh
      if (conversationId && conversationId !== Constants.NEW_CONVO) {
        console.log(
          '🔄 [useArtifacts] STEP 2: Loading conversation cache from database for:',
          conversationId,
        );
        try {
    ...
    loadCache();
  }, [conversationId, artifactsContext.conversationId, location.pathname]);
  useEffect(() => {
    ...
    Object.keys(artifacts).forEach((artifactId) => {
      ...
      // CRITICAL: Skip hydration for update artifacts - they contain delta content only
      // The component (Artifact.tsx) will compute the full merged content using useMemo
  ...
  useEffect(() => {
    ...
    if (isConversationChange && conversationId !== Constants.NEW_CONVO) {
      ...
      console.log('✅ [useArtifacts] Cache is ready - proceeding to load artifacts');
  ...
  const currentDisplayArtifact = useMemo(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentArtifactId, artifacts, _refreshTrigger]); // CRITICAL: _refreshTrigger is intentional to force refresh
  ...
  useEffect(() => {
    ...
    console.log(
      '✅✅✅ [useArtifacts] Message streaming complete - checking for updates',
      latestMessageText,
    );
  ...
  return {
    ...
    cacheLoaded, // Export cache loading state
  };
}
